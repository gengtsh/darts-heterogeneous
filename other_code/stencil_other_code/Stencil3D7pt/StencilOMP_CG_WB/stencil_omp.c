...
//stencil 3D7pt sequential code with blocking 
void stencil3D7pt_seq_wb ( double* __restrict__ dst,    double* __restrict__ src, const size_t     n_rows, const size_t     n_cols,const size_t     n_slices,const size_t     n_tsteps )
{
	...
	int tile_z = (n_slices>GRID_TILECPU_Z)?GRID_TILECPU_Z:n_slices;
   
    int64_t pos;
    for (size_t ts = 0; ts < n_tsteps; ++ts) {
        for (size_t k = 0; k< n_slices;k=k+tile_z){
	    	for (size_t i = 0; i < n_rows; i=i+tile_y) {
                for (size_t j = 0; j < n_cols; j=j+tile_x) {
                    pos = k*n_rows*n_cols+i*n_cols+j;
                    int n_slices_ck = ((k+tile_z+1)>n_slices) ?(n_slices-k-1):(tile_z+1);
                    int n_rows_ck   = ((i+tile_y+1)>n_rows)   ?(n_rows-i-1  ):(tile_y+1);
                    int n_cols_ck   = ((j+tile_x+1)>n_cols)   ?(n_cols-j-1  ):(tile_x+1);
                    computeBlock_stencil37(dst+pos,src+pos,n_rows,n_cols,n_slices,n_rows_ck,n_cols_ck,n_slices_ck);
                }
            }
        }
        swap_ptr(&dst,&src);
