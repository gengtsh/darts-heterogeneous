...
result_is_correct(const size_t  n_rows, const size_t  n_cols, 
                  const double* restrict orig,   const double* restrict res,   const double* restrict init)
{
    typedef double (*Array2D)[n_cols];
    const Array2D ORIG       = (const Array2D) orig,
                  RES        = (const Array2D) res;
                  // INIT        = (const Array2D) init;
    bool          is_correct =   true;
    const double  epsilon    = 0.1;

    for (size_t i = 0; is_correct && i < n_rows; ++i) 
		for (size_t j = 0; is_correct && j < n_cols; ++j) {
			// fprintf(stderr, "O[%d][%d] = %f, R[%d][%d] = %f, I[%d][%d] = %f\n", i, j, ORIG[i][j], i, j, RES[i][j], i, j, INIT[i][j]);
...
{
	typedef double (*Array2D)[n_cols];
    double *dst = (double*)smalloc ( SIZE2D(double, n_rows, n_cols) ),
           *src = (double*)smalloc ( SIZE2D(double, n_rows, n_cols) );
    array2d_init ( src, n_rows, n_cols );
    memcpy ( dst, src, sizeof(double) * n_rows * n_cols );
    stencil->arg = (stencil_arg_t*)smalloc ( SIZE2D( double, n_rows, n_cols ) );
...
{
    *dest = (stencil_arg_t) { 
        .dst     = (double*)NULL,          .src    = (double*)NULL, 
        .n_rows  = sourc->n_rows, .n_cols = sourc->n_cols, 
        .n_tsteps = sourc->n_tsteps 
    };
    dest->src = (double*)smalloc ( SIZE2D(double,dest->n_rows,dest->n_cols) );
    dest->dst = (double*)smalloc ( SIZE2D(double,dest->n_rows,dest->n_cols) );
...
run_kernel(code_t code, stencil_t *stencil, const size_t n_reps, const char* label)
{
    ...
    pthread_t timer_thd;
    spthread_create ( &timer_thd, (pthread_attr_t*)NULL /* no attributes */, timer, &loc_timer );
    if ( 0 != pthread_join ( timer_thd, (void**)NULL ) )
...
main(int argc, char* argv[])
{
    ...
    switch ( argc ) {
    case 5: n_reps     = strtoul(argv[4], (char**)NULL,0);
    case 4: n_tm_steps = strtoul(argv[3], (char**)NULL,0); 
    case 3: n_rows     = strtoul(argv[1], (char**)NULL,0); 
            n_cols     = strtoul(argv[2], (char**)NULL,0);
            break;
    case 2: n_rows = n_cols = strtoul(argv[1], (char**)NULL,0);
            break;
...
////===
    ...
    long  n_threads     = str_n_threads ? strtoul(str_n_threads,(char**)NULL,0) : 1L;
