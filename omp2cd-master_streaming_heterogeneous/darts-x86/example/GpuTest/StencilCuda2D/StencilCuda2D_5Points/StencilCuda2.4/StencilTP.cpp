...
Stencil2D4ptGpuLoopCD::fire(void)
{
	...
	cudaError err1,err2,err3,err4,err5,err6,err7,err8;
	err1 = cudaMalloc( (void **) &d_dst, d_size);
	err2 = cudaMalloc( (void **) &d_src, d_size);
	...
	if(err4!=cudaSuccess){
		std::cout<<"GpuLoop: cuda memcpyHostToDevice d_src: "<<cudaGetErrorString(err4)<<std::endl;
		exit(-1);
	}
#endif
	...
	int tile_y = ((nRows-2)>GRID_TILE_Y)?GRID_TILE_Y:(nRows-2);
	int gridDimx = (nCols-2)/blockDimx;
	int gridDimy = (nRows_bk-2)/tile_y; 
	int blockDimx_u = (nCols-2)%blockDimx;
	int tile_y_u = (nRows_bk-2)%tile_y;
	int gridDimx_u=(blockDimx_u)?1:0;
	int gridDimy_u= (tile_y_u)?1:0;
	...
	std::cout<<"GpuLoop["<<Id<<"]: gridDimx_u="<<gridDimx_u<<",gridDimy_u="<<gridDimy_u<<std::endl;
	std::cout<<"GpuLoop["<<Id<<"]: tile_y:"<<tile_y<<std::endl;
	std::cout<<"GpuLoop["<<Id<<"]: tile_y_u:"<<tile_y_u<<std::endl;
	std::cout<<"GpuLoop["<<Id<<"]: blockDimx:"<<blockDimx<<std::endl;
	std::cout<<"GpuLoop["<<Id<<"]: blockDimx_u:"<<blockDimx_u<<std::endl;
	...
	dim3 dimGrid(gridDimx,gridDimy);
	dim3 dimBlock(blockDimx,blockDimy);
	gpu_kernel5(dimGrid,dimBlock,d_dst,d_src,tile_y,0,0,nRows_bk,nCols,FRAME(stream[0]));
	if(gridDimx_u){
		pos_col = gridDimx * blockDimx; 
		dim3 dimGrid(gridDimx_u,gridDimy);
		dim3 dimBlock(blockDimx_u,blockDimy);
		if(blockDimx_u==1){
			gpu_kernel5_thread1(dimGrid,dimBlock,d_dst,d_src,tile_y,0,pos_col,nRows_bk,nCols,FRAME(stream[1]));
		}else{
			gpu_kernel5(dimGrid,dimBlock,d_dst,d_src,tile_y,0,pos_col,nRows_bk,nCols,FRAME(stream[1]));
		}
	}
	if(gridDimy_u){
		pos_row = gridDimy * tile_y; 
		dim3 dimGrid(gridDimx,gridDimy_u);
		dim3 dimBlock(blockDimx,blockDimy);
		gpu_kernel5(dimGrid,dimBlock,d_dst,d_src,tile_y_u,pos_row,0,nRows_bk, nCols,FRAME(stream[2]));
	}
	if(gridDimx_u && gridDimy_u){
		pos_row = gridDimy * tile_y; 
		pos_col = gridDimx * blockDimx; 
		dim3 dimGrid(gridDimx_u,gridDimy_u);
		dim3 dimBlock(blockDimx_u,blockDimy);
		if(blockDimx_u==1){
			gpu_kernel5_thread1(dimGrid,dimBlock,d_dst,d_src,tile_y_u,pos_row,pos_col,nRows_bk,nCols,FRAME(stream[3]));
		}else{
			gpu_kernel5(dimGrid,dimBlock,d_dst,d_src,tile_y_u,pos_row,pos_col,nRows_bk,nCols,FRAME(stream[3]));
		}
	}
