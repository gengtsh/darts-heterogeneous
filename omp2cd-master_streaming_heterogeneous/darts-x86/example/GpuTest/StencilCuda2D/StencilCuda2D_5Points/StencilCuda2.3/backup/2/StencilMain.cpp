...
int main(int argc, char *argv[])
{
	...
	if (affin.generateMask()){
		...
		for(size_t i=0; i<nReps;++i){
			...
			rt->run(
                launch<StencilTP>(InitialMatrix,nRows,nCols,NewMatrix,nTmSteps,hard, &Runtime::finalSignal)
            );
...
//              << outerAvg                << "\n";
	...
	StencilSeq ( SeqMatrix,OriginalMatrix,nRows,nCols,nTmSteps); 

	if(result_is_correct(nRows,nCols,SeqMatrix,NewMatrix)){
