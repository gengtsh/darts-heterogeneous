...
{
	std::cout<<"CalcTimeConstraintsForElems is running, CalcCourantConstraintForElemsTP and CalcHydroConstraintForElemsTP begin! "<<std::endl;
...
//	}
	   ...
	   /* evaluate time constraint */
		INVOKE(CalcCourantConstraintForElemsTP, domain, courant_elem_per_thread, dtcourant_per_thread,&FRAME(calcTimeConstraintsForElemsSync )) ;
	
	   /* check hydro constraint */
	   INVOKE(CalcHydroConstraintForElemsTP,domain, hydro_elem_per_thread, dthydro_per_thread,&FRAME(calcTimeConstraintsForElemsSync)) ;
...
{
	std::cout<<"CalcTimeConstraintsForElemsSync is running!"<<std::endl;
	LOAD_FRAME(CalcTimeConstraintsForElemsTP);
	SIGNAL(signalUp);
