...
{
	//std::cout<<"CalcLagrangeElements finish!"<<std::endl;	
	LOAD_FRAME(LagrangeElementsTP);
	Domain *domain=FRAME(domain);
	Real_t *vnew = FRAME(vnew) ; 
...
void ApplyMaterialPropertiesForElemsCD::fire(void)
{
	...
	//std::cout<<"CalcQForElems finish!"<<std::endl;	
	LOAD_FRAME(LagrangeElementsTP);
	Domain *domain=FRAME(domain);
	Real_t *vnew = FRAME(vnew) ; 
...
{
	//std::cout<<"ApplyMaterialPropertiesForElems finish!"<<std::endl;
	LOAD_FRAME(LagrangeElementsTP);
	Domain *domain=FRAME(domain);
	Real_t *vnew = FRAME(vnew) ; 
...
{
	//std::cout<<"UpdateVolumesForElems finish!"<<std::endl;
	LOAD_FRAME(LagrangeElementsTP);
//	Domain *domain=FRAME(domain);
//	Index_t numElem=domain->numElem();
//	Real_t *vnew = FRAME(vnew) ; 
