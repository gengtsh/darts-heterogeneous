...
//      std::cout << "region" << i + 1<< "size" << locDom->regElemSize(i) <<std::endl;
	...
	uint64_t *timings= new uint64_t [opts.its*opts.rep];
    Runtime* rt;
	...
	if (affin.generateMask()){
		...
		int its=0;
		for(int idx=0;idx<opts.rep;++idx){

			Domain *locDom ;
			// Build the main data structure and initialize it
			locDom = new Domain(numRanks, col, row, plane, opts.nx, side, opts.numReg, opts.balance, opts.cost) ;
...
//				}
	...
	for(Int_t i=0;i < opts.its*opts.rep;++i){
		printf("%lu,",timings[i]);
	}
	printf("\n");
	delete rt;
