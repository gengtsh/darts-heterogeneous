...
my $mz_ed=36000;
my $mz_sp=1000;
for (my $mz=$mz_st;$mz<$mz_ed;$mz=$mz+$mz_sp){
	push @msize,$mz;
}
				...
				for my $msz (@msize){
					...
					for my $gb (@gbase){
						...
						for my $ra (@ratio){
							...
							my $filename = $k . '_' . $msz . '_' . $n_cu . '_' . $n_su .'_'.$nts.'_'.$gb.'_'.$ra. '.txt';
					...
					$nts_time{$msz} = \%msz_time;
				}
				$nthread_time{$nts} = \%nts_time;				
			}
...
push @exe_info, [@hwinfo,'Total_Workload','timestep','nthreads','Initial_GPU_Workload','CPU/GPU_Workload_Ratio','Exe_Time'];

my @exe_info_best=();
#push @exe_info_best, [@hwinfo,'Total_Workload','Initial_GPU_Workload','CPU/GPU_Workload_Ratio'];
push @exe_info_best, [@hwinfo,'Total_Workload','timestep','nthreads','Initial_GPU_Workload','CPU/GPU_Workload_Ratio'];
		...
		for my $nth (@nthreads){
			for my $nts (@ntimestep){
				for my $msz (@msize){
					for my $gb (@gbase){					
						for my $ra (@ratio){
						my $etime=$all_exe_time{$server}->{$k}->{$nth}->{$nts}->{$msz}->{$gb}->{$ra};
							if ($etime!=0){
								push @exe_info, [@v_hw_info,$msz,$nts,$nth,$gb,$ra,$etime];
							}
						}
					}
				}
		...
		for my $msz (@msize){
			...
			for my $nts(@ntimestep){
				for my $nth (@nthreads){
					for my $gb (@gbase){					
						for my $ra (@ratio){		
							push @exe_msz, [$nts,$nth,$gb,$ra,$all_exe_time{$server}->{$k}->{$nth}->{$nts}->{$msz}->{$gb}->{$ra}];
						}
					}
				}
				...
				if(@array){
					push @exe_info_best,[@v_hw_info,$msz,$array[0][0],$array[0][1],$array[0][2],$array[0][3]];
				}
			}
...
my $file_out = $strongOrweak.'_'.'st_execution3'.'.csv';
open my $fh, '>', $file_out or die "couldn't open $file_out: $!";
print $fh join(',',@$_) . "\n" for (@exe_info);

my $file_out2 = $strongOrweak.'_'.'st_execution3_best'.'.csv';
