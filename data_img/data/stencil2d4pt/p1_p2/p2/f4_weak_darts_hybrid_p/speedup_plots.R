...
#for(j in c("ccsl", "debian", "f4", "hive", "supermicro")){
{
    ...
    j <-"StencilCudaHybrid4"
    sz<-23000
    
    #data <- read.csv(paste("./", j, "_", threads, "_1_weak_execution.dat", sep=""), header = T, sep = ",")
    data <- read.csv(paste("./", j,"_",sz,"_weak_execution.dat", sep=""), header = T, sep = ",")
    names(data) <- c("ratio","nCores", "1000", "2000", "4000", "8000")
    
    
    #if(j == "f4") j <- "Fatnode"
    jj <-"Fatnode"
    dataFrame <- data.frame(ratio=data$ratio, machine=jj, apps=c(rep(names(data)[3], dim(data)[1]),
                                                              rep(names(data)[4], dim(data)[1]),
                                                              rep(names(data)[5], dim(data)[1]),
                                                              rep(names(data)[6], dim(data)[1])),
                            exetime=c(data[,3], data[,4], data[,5], data[,6]))
...
Graph <- ggplot(data=df, aes(x=ratio, y=exetime, group=apps, col=apps, pch=apps, linetype = apps)) + 
  geom_line(size=1.5)+
  geom_point(cex=3.5) +
  xlab("Ratio") + 
  theme_bw() +
...
ggsave(paste("./cg_ratio.pdf",sep=""), Graph, device = pdf, height=9, width=9)
