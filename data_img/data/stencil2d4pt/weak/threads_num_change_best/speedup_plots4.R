...
for(j in c( "f4")){  
    ...
    names(data) <- c("size", "CPU-Sequence", "GPU-only", "DARTS-CPU", "DARTS-GPU", "DARTS-IDAWL")
...
df$apps <- factor(df$apps, levels = c("CPU-Sequence", "DARTS-IDAWL", "DARTS-CPU", "DARTS-GPU", "GPU-only"))
...
ggsave(paste("./speedUp_new3.pdf",sep=""), Graph, device = pdf, height=30, width=30)
...
ggsave(paste("./speedUpZoom_new4_fatnode.pdf",sep=""), Graph, device = pdf, height=30, width=30)