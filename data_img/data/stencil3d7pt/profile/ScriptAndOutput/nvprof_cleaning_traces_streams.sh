...
for machine in "${Machines[@]}"; do
    ...
    for sizesZXY in "${ZXY[@]}"; do
        ...
        for iter in `seq 1 9`; do
               ...
               tempTraceValues=`grep  "${metric}" ../data/"${machine}/${stream_version}/nvprof_trace_StencilCudaGpu37_${sizesYXZ}_${coreThreadMachine}_${iter}.txt" | sed -e 's/,/ /g' |  awk '{print $2}' | Rscript -e 'd<-scan("stdin", quiet=TRUE)' -e 'cat(length(d),  min(d), max(d), mean(d), sum(d), sep=",")'`
       	    ...
       	    for metric_total in "${total_times_traces[@]}"; do
                if [[ "${metric_total}" == "total_CUDA memcpy HtoD" ]]; then
                        tempTraceValues=`grep  "CUDA memcpy HtoD" ../data/"${machine}/${stream_version}/nvprof_trace_StencilCudaGpu37_${sizesYXZ}_${coreThreadMachine}_${iter}.txt" | sed -e 's/,/ /g' |  awk '{print $2}' | Rscript -e 'd<-scan("stdin", quiet=TRUE)' -e 'cat(length(d),  min(d), max(d), mean(d), sum(d), sep=",")'`
                fi

                    if [[ "${metric_total}" == "total_CUDA memcpy DtoH" ]]; then
                        tempTraceValues=`grep  "CUDA memcpy DtoH" ../data/"${machine}/${stream_version}/nvprof_trace_StencilCudaGpu37_${sizesYXZ}_${coreThreadMachine}_${iter}.txt" | sed -e 's/,/ /g' |  awk '{print $2}' | Rscript -e 'd<-scan("stdin", quiet=TRUE)' -e 'cat(length(d),  min(d), max(d), mean(d), sum(d), sep=",")'`
                    fi  

                    if [[ "${metric_total}" == "total_kernel_execution_time" ]]; then
                        K1=`grep  "${Traces[13]}" ../data/"${machine}/${stream_version}/nvprof_trace_StencilCudaGpu37_${sizesYXZ}_${coreThreadMachine}_${iter}.txt" | sed -e 's/,/ /g' |  awk '{print $2}' | Rscript -e 'd<-scan("stdin", quiet=TRUE)' -e 'cat(sum(d), sep=",")'`
                        K2=`grep  "${Traces[15]}" ../data/"${machine}/${stream_version}/nvprof_trace_StencilCudaGpu37_${sizesYXZ}_${coreThreadMachine}_${iter}.txt" | sed -e 's/,/ /g' |  awk '{print $2}' | Rscript -e 'd<-scan("stdin", quiet=TRUE)' -e 'cat(sum(d), sep=",")'`
                        K3=`grep  "${Traces[17]}" ../data/"${machine}/${stream_version}/nvprof_trace_StencilCudaGpu37_${sizesYXZ}_${coreThreadMachine}_${iter}.txt" | sed -e 's/,/ /g' |  awk '{print $2}' | Rscript -e 'd<-scan("stdin", quiet=TRUE)' -e 'cat(sum(d), sep=",")'`
                        K4=`grep  "${Traces[19]}" ../data/"${machine}/${stream_version}/nvprof_trace_StencilCudaGpu37_${sizesYXZ}_${coreThreadMachine}_${iter}.txt" | sed -e 's/,/ /g' |  awk '{print $2}' | Rscript -e 'd<-scan("stdin", quiet=TRUE)' -e 'cat(sum(d), sep=",")'`
    ...
    sed -i -e 's/\"//g' tempFileMetric 
    mv tempFileMetric GPUExecutionTimeSAndNS/nvprofTraces_${machine}_${stream_version}.csv     
done
