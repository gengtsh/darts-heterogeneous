...
if (not defined $sv) {
  die "Need sv: f3 or ccsl or debian or supermicro\n";
}
			...
			for(my $IsStream=0;$IsStream<2;++$IsStream){
				...
				for(my $m=1;$m<$n_iter;++$m){
					...
					if($IsStream==0){
						...
						my $filename = $prefix.'_'.${i}.'_'.${j}.'_'.${k}.'_'.${n_cu}.'_'.${n_su}.'_'.$m.'_'.${n_reps}.'_'.${IsStream}.'_'.${nt}.'.txt';
...
###################print output##############################
my $postpose = "postpose_weak_execution_".$sv;
my @output_server;

my @out_features;
push @out_features,"machine","kernel","size(xyz)","iteration","IsStream","nTile","totalExeTime";
			...
			for(my $IsStream=0;$IsStream<2;++$IsStream){
				for(my $m=1;$m<$n_iter;++$m){
					if($IsStream==0){
						...
						my @out_val;
						my $val;
						#print $hash_table{$sv}->{$kernel}->{$sz}->{$IsStream}->{$m}->{0},"*******************";
						if (exists $hash_table{$sv}->{$kernel}->{$sz}->{$IsStream}->{$m}->{0}){
							$val = $hash_table{$sv}->{$kernel}->{$sz}->{$IsStream}->{$m}->{0};
							push @out_val,$sv,$kernel,$xyz,$m,$IsStream,$nt,$val;
                        ...
                        for(my $nt=1;$nt<$ntVal+1;++$nt){
							my @out_val;
							my $val;
							if (exists $hash_table{$sv}->{$kernel}->{$sz}->{$IsStream}->{$m}->{$nt}){
								$val = $hash_table{$sv}->{$kernel}->{$sz}->{$IsStream}->{$m}->{$nt};
								push @out_val,$sv,$kernel,$xyz,$m,$IsStream,$nt,$val;
								push @output_server,[@out_val];
