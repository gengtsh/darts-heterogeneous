...
#  scale_x_continuous(breaks=seq(0,50000,5000)) +
  theme(axis.text.x= element_text(family = "Times", face="bold", size=50, colour = "Black", angle=35, hjust=0.85)) +
...
#  facet_grid(.~machine, ncol=1,scales="free") +
  facet_wrap(~machine, ncol=1, scales="free") +
  theme(strip.text = element_text(size=50))
ggsave(paste("./speedUp3-Stencil3D_debian.pdf",sep=""), Graph, device = pdf, height=30, width=30)
