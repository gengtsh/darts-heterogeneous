...
extern "C"
void
Stencil2D4ptGpuKernelWithStreamsCD::fire(void)
{

	LOAD_FRAME(StencilTP);
    RESET(Stencil2D4ptGpuKernelWithStreams);
#ifdef CUDA_DARTS_DEBUG
	std::cout<<"Invoke GpuKernelWithStreams"<<std::endl;
#endif
...
//	dim3 dimGrid(gridDimx,gridDimy);
	...
	std::cout<<std::setprecision(18)<<std::endl;
	std::cout<<"GpuKernelWithStreams: gpu memory total: "<<gpu_mem_total_t<<std::endl;
	std::cout<<"GpuKernelWithStreams: gpu memory available: "<<gpu_mem_avail_t<<std::endl;
	std::cout<<"GpuKernelWithStream : require memory size:"<<d_size + d_size_sharedCols + d_size_sharedRows<<std::endl;
	std::cout<<"GpuKernelWithStreams: Gpupos:"<<FRAME(gpuPos)<<std::endl;
	std::cout<<"GpuKernelWithStreams: d_size:"<<d_size<<std::endl;
	std::cout<<"GpuKernelWithStreams: nRowsGpuBlock:"<<nRowsGpuBlock<<std::endl;
	std::cout<<"GpuKernelWithStreams: chunk:"<<chunk<<std::endl;
#endif
...
#ifdef CUDA_ERROR_CHECKING
    if(err1!=cudaSuccess){
        std::cout<<"GpuKernelWithStreams: cuda malloc d_dst: "<<cudaGetErrorString(err1)<<std::endl;
        exit(-1);
    }
    if(err2!=cudaSuccess){
        std::cout<<"GpuKernelWithStreams: cuda mallock d_sharedRows: "<<cudaGetErrorString(err2)<<std::endl;
        exit(-1);
    }
    
    if(err3!=cudaSuccess){
        std::cout<<"GpuKernelWithStreams: cuda mallock d_sharedCols: "<<cudaGetErrorString(err3)<<std::endl;
        exit(-1);
    }
#endif
    ...
    while(ts-- >0){
        for (size_t i = 0; i<nGPU; ++i){
            for (size_t j =0; j<nStream;++j){
                ...
                err3 = cudaGetLastError();
                if(cudaSuccess != err3){
                    std::cout<<"GpuKernelWithStreams multiple streams: kernel5 stream error: "<<cudaGetErrorString(err3)<<std::endl;
                    exit(-1);
                }
#endif

#ifdef CUDA_DARTS_DEBUG
        	    std::cout<<"GpuKernelWithStreams multiple streams: invoke stream["<<j<<"]"<<",ps:"<<ps<<std::endl;
#endif

                d_size_stream = sizeof(double)*nCols*nRowsGpuStream;
#ifdef CUDA_DARTS_DEBUG
                std::cout<<"GpuKernelWithStreams multiple streams: stream["<<j<<"]"<<",ps:"<<ps<<", size:"<<d_size_stream<<",d_pos:"<<d_pos/nCols<<",h_pos:"<<h_pos/nCols<<",nRowsGpuStream:"<<nRowsGpuStream<<std::endl;
#endif
...
#ifdef CUDA_ERROR_CHECKING
	    if(err4!=cudaSuccess){
		    std::cout<<"GpuKernelWithStreams: cuda deviceSynchronize: "<<cudaGetErrorString(err4)<<std::endl;
		    exit(-1);
	    }
#endif
	    SWAP_PTR(&h_dst ,&h_src);
    ...
    if(err1!=cudaSuccess){
	    std::cout<<"GpuKernelWithStreams: cuda memcpy free d_dst: "<<cudaGetErrorString(err1)<<std::endl;
    	exit(-1);
    }

    if(err2!=cudaSuccess){
	    std::cout<<"GpuKernelWithStreams: cuda memcpy free d_sharedRows: "<<cudaGetErrorString(err2)<<std::endl;
    	exit(-1);
    }

    if(err3!=cudaSuccess){
	    std::cout<<"GpuKernelWithStreams: cuda memcpy free d_sharedCols: "<<cudaGetErrorString(err3)<<std::endl;
    	exit(-1);
    }
#endif
    if(FRAME(tsInit%2)){
