...
__global__ void gpu_stencil2D_4pt_hack5(double * dst, double * src,int tile_y,int rowPos,int colPos, int M, int N)
{
	...
	int base_global_row = rowPos + ( tile_y* blockIdx.y ); 
	int base_global_col = colPos + blockDim.x*blockIdx.x;
...
//		if(blockDim.x==1){
//			shared_mem[j+blockDim.x+1]=src[k+blockDim.x+1];
//		}
	}
	...
	for ( curRow = HALO; curRow < tile_y; curRow+=1 )
	{
		...
		shared_mem [north*(blockDim.x+2)+t] =src[k];
		if(t<2){
			shared_mem[north*(blockDim.x+2)+t+blockDim.x]=src[k+blockDim.x];
		}	
		
//		if(blockDim.x==1){
//			shared_mem[north*(blockDim.x+2)+t+blockDim.x+1]=src[k+blockDim.x+1];
//		}	
	...
	if(threadIdx.x==0 && blockIdx.x ==0 ){
		printf("tile_y= %d, last row=%d \n",tile_y, base_global_row + curRow);	
		printf("blockDim.x= %d, first cols=%d \n",blockDim.x, base_global_col + threadIdx.x+1);	
	}
...
__global__ void gpu_stencil2D_4pt_hack5_thread1(double * dst, double * src,int tile_y,int rowPos,int colPos, int M, int N)
{
#ifdef CUDA_DARTS_DEBUG
	if((blockIdx.x==0)&&(blockIdx.y==0)&&(threadIdx.x==0)){
		printf("kernel begin!\n");
...
void gpu_kernel5(dim3 dimGrid,dim3 dimBlock,double * d_dst, double * d_src,int tile_y,int rowPos,int colPos,  int M, int N,cudaStream_t &stream){
...
#endif
		gpu_stencil2D_4pt_hack5<<<dimGrid,dimBlock,sharedMemSize,stream>>>(d_dst,d_src,tile_y,rowPos,colPos,M,N);
#ifdef CUDA_DARTS_DEBUG
		printf("gpu kernel return to host, but kernel haven't finished!\n");
#endif
}

void gpu_kernel5_thread1(dim3 dimGrid,dim3 dimBlock,double * d_dst, double * d_src,int tile_y,int rowPos,int colPos,  int M, int N,cudaStream_t &stream){
		int sharedMemSize = sizeof(double)*(1+HALO*2)*(dimBlock.x+2);
#ifdef CUDA_DARTS_DEBUG
		printf("sharedMemSize: %dKB, total sharedMemSize: %dKB\n",sharedMemSize/1024, sharedMemSize*dimGrid.x*dimGrid.y/1024);
#endif
		gpu_stencil2D_4pt_hack5_thread1<<<dimGrid,dimBlock,sharedMemSize,stream>>>(d_dst,d_src,tile_y,rowPos,colPos,M,N);
#ifdef CUDA_DARTS_DEBUG
		printf("gpu kernel return to host, but kernel haven't finished!\n");
#endif
}
