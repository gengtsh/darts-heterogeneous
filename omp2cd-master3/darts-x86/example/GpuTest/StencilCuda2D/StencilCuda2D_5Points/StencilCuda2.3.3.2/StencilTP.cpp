...
Stencil2D4ptCpuLoopCD::fire(void)
{
    ...
    size_t cpuPos          = (cpuPosx+cpuPosy*nCols);
	
    int posy = Id/cpuGridDimx;
	int posx = Id - posy*cpuGridDimx;

	size_t pos = posy*cpuBlockDimy*nCols + posx*cpuBlockDimx;
...
void Stencil2D4ptCpuSyncCD::fire(void)
{
    ...
    if(nRowsLeft==0.0){
		...
		if(FRAME(nRowsLeft)==0){
			...
			std::cout<<"CpuSync: nRowLeft:"<<FRAME(nRowsLeft)<<std::endl;
#endif
            ...
            std::cout<<"CpuSync: invoked  new CPU, nCPU: "<<FRAME(nCPU)<<std::endl;
