...
DEF_TP(StencilTP)
{
    ...
    StencilTP(double * inimatrix,const uint64_t inim,const uint64_t inin,double * newmatrix,uint64_t ts,bool hard,bool IsStatic,double GpuRatio, Codelet *up)
	:Initial(inimatrix)
	,nRows(inim)
	,nCols(inin)
	,New(newmatrix)
				...
				if (hard == true){
				    ...
				    if(req_size <  gpuMemMax){
					    ...
					    if(IsStatic == false){
                            ...
                            CpuLoop = new Stencil2D4ptCpuLoopCD[nCPU];
                            for(size_t i=0;i<nCPU; ++i){
	    	   	                CpuLoop[i] = Stencil2D4ptCpuLoopCD{1,1,this,SHORTWAIT,i};
	    	                }
                        ...
                        //==== calculate CpuLoop pos ====//
                        arrnCpuPos[1] = cpuPos;

                        //==== calculate num of CpuLoop for nRowsCpu ====//
                        arrnCpuEdge[0] = nCols;
