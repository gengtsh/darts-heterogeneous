...
DEF_TP(StencilTP)
{
    ...
    StencilTP(double * inimatrix,const uint64_t n_rows,const uint64_t n_cols, const uint64_t n_slices,double * newmatrix,uint64_t ts,bool hard,double GpuRatio,bool streamming,int32_t nTiles, Codelet *up)
	:Initial(inimatrix)
	,nRows(n_rows)
	,nCols(n_cols)
    ,nSlices(n_slices)
                ...
                if (GpuRatio == 1.0){
                   ...
                   // if((streamming==false)&&(req_size<gpuMemMax)){
                    if(streamming==false){
                        ...
                        //ncut = (nGPU==1)?2:nGPU;
                        //vnStream = 1;
                        //nId=std::min_element(arrnGpuGrid,arrnGpuGrid+DIM)-arrnGpuGrid;
                        //
                        //int gridBig ;                          
                        ////if(arrnGpuGrid[nId]*ncut < std::min(arrnGpuGrid[R(nId+1)],arrnGpuGrid[R(nId-1)])){
                        ////    gridBig = arrnGpuGrid[nId];
                        ////}else{
                        ////    gridBig = std::ceil(1.0*arrnGpuGrid[nId]/ncut);                         
                        ////}
                        //
                        //gridBig = std::ceil(1.0*arrnGpuGrid[nId]/ncut);                          
                        //int maxGrid=0;
                        //for(int k = 0; k<DIM;++k){
                        //    int vn = std::ceil(1.0*arrnGpuGrid[k]/gridBig);
                        //    vnStream *=vn;
                        //    maxGrid = (maxGrid > vn)? maxGrid:vn; 
                        //    arrnGpuGrid[k]=gridBig;
                        //}
                        //gnStream = maxGrid; 
#ifdef CUDA_DARTS_DEBUG
                    ...
                    if(req_size< gpuMemMax){
                        ...
                        //int64_t gx = arrnEdge[0];
                        //if(std::all_of(arrnEdge,arrnEdge+DIM,[gx](int64_t x){return x==gx;})){
                        //    nId = 2;
                        //}else{
