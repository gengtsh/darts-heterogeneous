...
DEF_TP(StencilTP)
{
    ...
    StencilTP(double * inimatrix,const uint64_t n_rows,const uint64_t n_cols, const uint64_t n_slices,double * newmatrix,uint64_t ts,bool hard,bool IsStatic,double GpuRatio, Codelet *up)
	:Initial(inimatrix)
	,nRows(n_rows)
	,nCols(n_cols)
    ,nSlices(n_slices)
...
//            }
            ...
            if(IsStatic == false){
                ...
                if((nSlices==400)&&(nRows==800)&&(nCols==800)){
	                gpuWLBase =100;
	                cpuWLBase =300;	
	               
                }else if((nSlices==800)&&(nRows==1000)&&(nCols==1000)){
                    gpuWLBase =100;
	                cpuWLBase =200;	
