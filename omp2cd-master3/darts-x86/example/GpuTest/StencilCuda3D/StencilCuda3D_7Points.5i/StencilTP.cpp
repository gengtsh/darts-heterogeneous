...
void Stencil3D7ptCpuSyncCD::fire(void)
{
    ...
    if( checkarrnEqValue(arrnCpuEdgeLeft, 0) && checkarrnEqValue(arrnGpuEdgeLeft,0)){//calculation finish
        ...
        //============================= calc cpu edge  =====================================//
            pthread_mutex_lock(&mutex);
            __sync_synchronize();
            calcNextEP(arrnCpuEdge, arrnGpuEdge,arrnCpuEdgeLeft, arrnGpuEdgeLeft,arrnCpuPos, arrnGpuPos,arrnEdge, arrnCpuEdgeVar, arrnGpuEdgeVar,arrnCpuEdgeMin,arrnGpuEdgeMin,cpuCnt,gpuCnt, nId,"cpu",FRAME(IsStatic));
            __sync_synchronize();
            pthread_mutex_unlock(&mutex);
...
Stencil3D7ptSwapCD::fire(void) 
{
	...
	if(GpuRatio == 0.0){
        ...
        if(ts!=0){
            ...
            //============================= calc cpu edge  =====================================//
            calcNextEEL(arrnCpuEdge, arrnCpuEdgeLeft,arrnEdge, arrnCpuEdgeVar,arrnCpuEdgeMin,nId, cpuCnt, gpuCnt,IsStatic);
            calcNextEEL(arrnCpuEdge, arrnCpuEdgeLeft,arrnEdge, arrnCpuEdgeVar,arrnCpuEdgeMin,nIdP1, cpuCnt, gpuCnt,IsStatic);
            calcNextEEL(arrnCpuEdge, arrnCpuEdgeLeft,arrnEdge, arrnCpuEdgeVar,arrnCpuEdgeMin,nIdM1, cpuCnt, gpuCnt,IsStatic);
            //============================= calc gpu rnId EdgeLeft = cpu rnId Edgeleft==========//
            arrnGpuEdgeLeft[nId] = arrnCpuEdgeLeft[nId];

            //============================= calc gpu edge  =====================================//
            calcNextEEL(arrnGpuEdge, arrnGpuEdgeLeft,arrnEdge, arrnGpuEdgeVar,arrnGpuEdgeMin,nId, gpuCnt, cpuCnt,IsStatic);
            calcNextEEL(arrnGpuEdge, arrnGpuEdgeLeft,arrnEdge, arrnGpuEdgeVar,arrnGpuEdgeMin,nIdP1, gpuCnt, cpuCnt,IsStatic);
            calcNextEEL(arrnGpuEdge, arrnGpuEdgeLeft,arrnEdge, arrnGpuEdgeVar,arrnGpuEdgeMin,nIdM1, gpuCnt, cpuCnt,IsStatic);
            //============================= calc gpu pos  =====================================//
...
extern "C"
void
Stencil3D7ptGpuKernelWithAllTimeStepsBigCD::fire(void)
{

#ifdef CUDA_DARTS_DEBUG
	std::cout<<"Invoke GpuKernelWithAllTimeStepsBig37!"<<std::endl;	
#endif
	LOAD_FRAME(StencilTP);
	RESET(GpuKernelWithAllTimeStepsBig37);
    ...
    int32_t nGPU = FRAME(nGPU);

	const uint64_t gpuCols   = FRAME(arrnGpuEdge[0]);
    const uint64_t gpuRows   = FRAME(arrnGpuEdge[1]);
    const uint64_t gpuSlices = FRAME(arrnGpuEdge[2]);
    const uint64_t gpuSlicesSub = std::ceil(1.0*gpuSlices/nGPU);
    const uint64_t gpuSlicesSub1 = gpuSlicesSub+1;
    const uint64_t gpuSlicesSub2 = gpuSlicesSub+2;
    

#ifdef CUDA_DARTS_DEBUG

	std::cout<<"GpuKernelWithAllTimeStepsBig: nGPU:"<<nGPU<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: gpuSlices:"<<gpuSlices<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: gpuSlicesSub1:"<<gpuSlicesSub1<<std::endl;
#endif
	const uint64_t nRows   = FRAME(nRows);
	const uint64_t nCols   = FRAME(nCols);
    ...
    int gpuGridDimx = FRAME(arrnGpuGrid[0]);
    int gpuGridDimy = FRAME(arrnGpuGrid[1]);
    int gpuGridDimz = FRAME(arrnGpuGrid[2]);
    int gpuGridDimzSub = std::ceil(1.0*gpuSlicesSub2/FRAME(arrnGpuTile[2])); 

    size_t szdb = FRAME(szdb);
    int64_t  d_size = szdb*gpuRows*gpuCols*gpuSlicesSub2;  
    int64_t d_size_sharedCols   = szdb*gpuRows*gpuSlicesSub2*gpuGridDimx*2;
    int64_t d_size_sharedRows   = szdb*gpuCols*gpuSlicesSub2*gpuGridDimy*2;
    int64_t d_size_sharedSlices = szdb*gpuRows*gpuCols*gpuGridDimzSub*2;


#ifdef CUDA_DARTS_DEBUG
	std::cout<<"GpuKernelWithAllTimeStepsBig: new d_size:"<<d_size<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: new d_size_sharedCols:"<<d_size_sharedCols<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: new d_size_sharedRows:"<<d_size_sharedRows<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: new d_size_sharedSlices:"<<d_size_sharedSlices<<std::endl;

    int64_t req_size = d_size + d_size_sharedCols + d_size_sharedRows + d_size_sharedSlices;
    size_t gpuMemMax = FRAME(gpuMemMax);
...
#ifdef CUDA_ERROR_CHECKING
	if(err1!=cudaSuccess){
		std::cout<<"GpuKernelWithAllTimeStepsBig: cuda malloc1: "<<cudaGetErrorString(err1)<<std::endl;
		exit(-1);
	}
	if(err2!=cudaSuccess){
		std::cout<<"GpuKernelWithAllTimeStepsBig: cuda malloc2: "<<cudaGetErrorString(err2)<<std::endl;
		exit(-1);
	}
	if(err3!=cudaSuccess){
		std::cout<<"GpuKernelWithAllTimeStepsBig: cuda malloc3: "<<cudaGetErrorString(err3)<<std::endl;
		exit(-1);
	}

	if(err4!=cudaSuccess){
		std::cout<<"GpuKernelWithAllTimeStepsBig: cuda malloc4: "<<cudaGetErrorString(err4)<<std::endl;
		exit(-1);
	}
#endif
...
#ifdef CUDA_ERROR_CHECKING
	        if(err[i]!=cudaSuccess){
	    	    std::cout<<"GpuKernelWithAllTimeStepsBig: cuda create event ["<<i<<"]:"<<cudaGetErrorString(err[i])<<std::endl;
	    	    exit(-1);
	        }
#endif
        }
	...
	size_t ts = FRAME(ts);
	while(ts-- >0){
        for(size_t k=0;k<nGPU;++k){

            gpuPosz = (gpuSlicesSub) * k;
            size_t gpuSlicesVal = (k==(nGPU-1))?(gpuSlices-k*gpuSlicesSub):(gpuSlicesSub2 );

            d_size = sizeof(double)*gpuRows*gpuCols*gpuSlicesVal;
            size_t  pos1 = gpuPosx+gpuPosy*gpuCols+gpuPosz*gpuRows*gpuCols;

#ifdef CUDA_DARTS_DEBUG	

            std::cout<<"GpuKernelWithAllTimeStepsBig: k: "<<k<<",d_size:  "<<d_size<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: k: "<<k<<",gpuPosz:  "<<gpuPosz<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: k: "<<k<<",pos1:  "<<pos1<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: k: "<<k<<",pos:  "<<h_src+pos1<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: k: "<<k<<",gpuSlicesVal:  "<<gpuSlicesVal<<std::endl;
#endif
	        err1 = cudaMemcpy(d_dst, h_src+pos1, d_size, cudaMemcpyHostToDevice);
#ifdef CUDA_ERROR_CHECKING
	        if(err1!=cudaSuccess){
		        std::cout<<"GpuKernelWithAllTimeStepsBig: cuda memcpyHostToDevice d_dst: "<<cudaGetErrorString(err1)<<std::endl;
		        exit(-1);
	        }
#endif
            ...
            int gpuGridDimz = std::ceil(1.0*gpuSlicesVal/gpuTile_z);     //  z/tile_z
...
#ifdef CUDA_DARTS_DEBUG
	        std::cout<<"GpuKernelWithAllTimeStepsBig: GpuPos:"<<pos1/gpuCols<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: nRows:"<<gpuRows<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: nCols:"<<gpuCols<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: nSlicesVal:"<<gpuSlicesVal<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: d_size:"<<d_size<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: d_size_sharedRows:"<<d_size_sharedRows<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: d_size_sharedCols:"<<d_size_sharedCols<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: d_size_sharedSlices:"<<d_size_sharedSlices<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: tile_x:"<<gpuTile_x<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: tile_y:"<<gpuTile_y<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: tile_z:"<<gpuTile_z<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: blockDimx:"<<gpuBlockDimx<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: blockDimy:"<<gpuBlockDimy<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: blockDimz:"<<gpuBlockDimz<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: gridDimx="<<gpuGridDimx<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: gridDimy="<<gpuGridDimy<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: gridDimz="<<gpuGridDimz<<std::endl;
            ...
            std::cout<<"GpuKernelWithAllTimeStepsBig: blockDimx_slices:"<<blockDimx_slices<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: blockDimy_slices:"<<blockDimy_slices<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: grimDimx_slices:"<<gridDimx_slices<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: grimDimy_slices:"<<gridDimy_slices<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: grimDimz_slices:"<<gridDimz_slices<<std::endl;
	        
            std::cout<<"GpuKernelWithAllTimeStepsBig: blockDimx_rows:"<<blockDimx_rows<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: blockDimy_rows:"<<blockDimy_rows<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: grimDimx_rows:"<<gridDimx_rows<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: grimDimy_rows:"<<gridDimy_rows<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: grimDimz_rows:"<<gridDimz_rows<<std::endl;
	        
            std::cout<<"GpuKernelWithAllTimeStepsBig: blockDimx_cols:"<<blockDimx_cols<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: blockDimy_cols:"<<blockDimy_cols<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: grimDimx_cols:"<<gridDimx_cols<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: grimDimy_cols:"<<gridDimy_cols<<std::endl;
	        std::cout<<"GpuKernelWithAllTimeStepsBig: grimDimz_cols:"<<gridDimz_cols<<std::endl;
            ...
            gpu_kernel37_cp_slices_stream(stream[0],dimGrid_slices,dimBlock_slices,d_dst,d_sharedRows, d_sharedCols, d_sharedSlices, gpuRows, gpuCols,gpuSlicesVal,gpuTile_x,gpuTile_y,gpuTile_z);
            err1=cudaEventRecord(cuEvent[0],stream[0]);
            gpu_kernel37_cp_rows_stream(stream[1],dimGrid_rows,dimBlock_rows,d_dst,d_sharedRows, d_sharedCols, d_sharedSlices, gpuRows, gpuCols,gpuSlicesVal,gpuTile_x,gpuTile_y,gpuTile_z);
            err2=cudaEventRecord(cuEvent[1],stream[1]);
            gpu_kernel37_cp_cols_stream(stream[2],dimGrid_cols,dimBlock_cols,d_dst,d_sharedRows, d_sharedCols, d_sharedSlices, gpuRows, gpuCols,gpuSlicesVal,gpuTile_x,gpuTile_y,gpuTile_z);
            err3=cudaEventRecord(cuEvent[2],stream[2]);
...
#ifdef CUDA_ERROR_CHECKING
	        if(err1!=cudaSuccess){
	        	std::cout<<"GpuKernelWithAllTimeStepsBig: cudaEventRecord stream0 errors: "<<cudaGetErrorString(err1)<<std::endl;
	        	exit(-1);
	        }
	        if(err2!=cudaSuccess){
	        	std::cout<<"GpuKernelWithAllTimeStepsBig: cudaEventRecord stream1 errors: "<<cudaGetErrorString(err2)<<std::endl;
	        	exit(-1);
	        }
	        if(err3!=cudaSuccess){
	        	std::cout<<"GpuKernelWithAllTimeStepsBig: cudaEventRecord stream2 errors: "<<cudaGetErrorString(err3)<<std::endl;
	        	exit(-1);
	        }
#endif
            err1=cudaStreamWaitEvent(stream[3],cuEvent[0],0);
...
#ifdef CUDA_ERROR_CHECKING
	        if(err1!=cudaSuccess){
	        	std::cout<<"GpuKernelWithAllTimeStepsBig: cudaStreamWaitEvent stream0 errors: "<<cudaGetErrorString(err1)<<std::endl;
	        	exit(-1);
	        }
	        if(err2!=cudaSuccess){
	        	std::cout<<"GpuKernelWithAllTimeStepsBig: cudaStreamWaitEvent stream1 errors: "<<cudaGetErrorString(err2)<<std::endl;
	        	exit(-1);
	        }
	        if(err3!=cudaSuccess){
	        	std::cout<<"GpuKernelWithAllTimeStepsBig: cudaStreamWaitEvent stream2 errors: "<<cudaGetErrorString(err3)<<std::endl;
	        	exit(-1);
	        }
#endif
				...
				gpuErrchk(err1);
#endif
            gpu_kernel37_stream(stream[3],dimGrid,dimBlock,d_dst,d_sharedRows,d_sharedCols,d_sharedSlices,gpuRows,gpuCols,gpuSlicesVal,gpuTile_x,gpuTile_y,gpuTile_z);

            err4=cudaEventRecord(cuEvent[3],stream[3]);

#ifdef CUDA_ERROR_CHECKING
	        if(err4!=cudaSuccess){
	        	std::cout<<"GpuKernelWithAllTimeStepsBig: cudaEventRecord stream3 errors: "<<cudaGetErrorString(err4)<<std::endl;
	        	exit(-1);
	        }
#endif
            err1=cudaStreamWaitEvent(stream[0],cuEvent[3],0);
...
#ifdef CUDA_ERROR_CHECKING
	        if(err1!=cudaSuccess){
	    	    std::cout<<"GpuKernelWithAllTimeStepsBig: cuda deviceSynchronize: "<<cudaGetErrorString(err1)<<std::endl;
	    	    exit(-1);
	        }
#endif
            
	        //copy from GPU  to CPU
            err1=cudaMemcpy(h_dst+pos1+gpuRows*gpuCols, d_dst+gpuRows*gpuCols,d_size-gpuRows*gpuCols*szdb, cudaMemcpyDeviceToHost);


#ifdef CUDA_DARTS_DEBUG

            std::cout<<"GpuKernelWithAllTimeStepsBig: pos1:"<<pos1<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: pos1+gpuRows*gpuCols:"<<pos1+gpuRows*gpuCols<<std::endl;
            std::cout<<"GpuKernelWithAllTimeStepsBig: posz:"<<gpuPosz+1<<std::endl;

#endif
#ifdef CUDA_ERROR_CHECKING
	        if(err1!=cudaSuccess){
	    	    std::cout<<"GpuKernelWithAllTimeStepsBig: cuda memcpyDeviceToHost: "<<cudaGetErrorString(err1)<<std::endl;
	    	    exit(-1);
	        }
#endif
        }
...
#ifdef CUDA_ERROR_CHECKING
	        if(err[i]!=cudaSuccess){
	    	    std::cout<<"GpuKernelWithAllTimeStepsBig: cuda destroy event ["<<i<<"]:"<<cudaGetErrorString(err[i])<<std::endl;
	    	    exit(-1);
	        }
#endif
        }
...
/*
	...
	std::cout<<"GpuKernelWithAllTimeStepsBig: nRows:"<<gpuRows<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: nCols:"<<gpuCols<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: nSlices:"<<gpuSlices<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: d_size:"<<d_size<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: d_size_sharedRows:"<<d_size_sharedRows<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: d_size_sharedCols:"<<d_size_sharedCols<<std::endl;
	std::cout<<"GpuKernelWithAllTimeStepsBig: d_size_sharedSlices:"<<d_size_sharedSlices<<std::endl;
...
#ifdef CUDA_ERROR_CHECKING
	if(err1!=cudaSuccess){
		std::cout<<"GpuKernelWithAllTimeStepsBig: cuda memcpy free d_dst: "<<cudaGetErrorString(err1)<<std::endl;
		exit(-1);
	}

	if(err2!=cudaSuccess){
		std::cout<<"GpuKernelWithAllTimeStepsBig: cuda memcpy free d_sharedCols: "<<cudaGetErrorString(err2)<<std::endl;
		exit(-1);
	}

	if(err3!=cudaSuccess){
		std::cout<<"GpuKernelWithAllTimeStepsBig: cuda memcpy free d_sharedRows: "<<cudaGetErrorString(err3)<<std::endl;
		exit(-1);
	}

	if(err4!=cudaSuccess){
		std::cout<<"GpuKernelWithAllTimeStepsBig: cuda memcpy free d_sharedRows: "<<cudaGetErrorString(err4)<<std::endl;
		exit(-1);
	}
#endif
...
Stencil3D7ptGpuKernelPureGpuWithStreamsCD::fire(void)
{
...
#ifdef CUDA_DARTS_DEBUG
    std::cout<<"new required size: "<<(t_d_size_sharedCols+t_d_size_sharedRows+t_d_size_sharedSlices+gnStream*dev_size*szdb)/KB<<"KB"<<std::endl;
#endif
    ...
    int maxNumStream = NUMSTREAMMAX; 
    int ttStream = (vnStream >maxNumStream)?maxNumStream:vnStream;
    int tnStream  = vnStream*nStream; 
    int ttnStream = ttStream*nStream;

#ifdef CUDA_DARTS_DEBUG
    std::cout<<"create stream number is "<<ttnStream<<std::endl;
#endif
    ...
    //for(int i=0;i<tnStream;++i){
	stream = new cudaStream_t[ttnStream];
    cudaEvent_t *cuEvent = new cudaEvent_t[ttnStream];
    cudaError *err = new cudaError[ttnStream];
    for(int i=0;i<ttnStream;++i){
        cudaStreamCreate(&stream[i]);
#ifdef CUDA_DARTS_DEBUG
...
#ifdef CUDA_DARTS_DEBUG
        std::cout<<"finish create stream!"<<std::endl;
#endif
...
//                }           
                ...
                int ii = i%ttStream;
                int pre = ((i+gnStream)%ttStream)*nStream;
                int idxSM=ii*nStream;
				int nSMM1=nStream-1;
				int idxSMM1 = idxSM+nSMM1;
				if((i/gnStream) !=0){
					//int pre = (i-gnStream)*nStream;
...
;
 ...
 (arrnSSGrid[2][0], arrnSSGrid[2][1],arrnSSGrid[2][2]);
        ...
        if((checkarrnEqValue(arrnCpuEdgeLeft, 0) && checkarrnEqValue(arrnGpuEdgeLeft,0))){
            ...
            pthread_mutex_lock(&mutex);
            __sync_synchronize();
            calcNextEP(arrnCpuEdge, arrnGpuEdge,arrnCpuEdgeLeft, arrnGpuEdgeLeft,arrnCpuPos, arrnGpuPos,arrnEdge, arrnCpuEdgeVar, arrnGpuEdgeVar,arrnCpuEdgeMin,arrnGpuEdgeMin,cpuCnt,gpuCnt, nId,"gpu",FRAME(IsStatic));
            __sync_synchronize();
            pthread_mutex_unlock(&mutex);
