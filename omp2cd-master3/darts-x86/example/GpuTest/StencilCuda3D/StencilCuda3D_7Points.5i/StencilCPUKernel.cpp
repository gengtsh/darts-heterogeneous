...
//calc next edge and edge left
void calcNextEEL(int * arrncgEdge, int *arrncgEdgeLeft,int *arrnEdge, int *varEdge,int *arrncgEdgeMin,int id, int homeCnt, int visitCnt,bool IsStatic){
        //int newEdge;
        //int newEdgeLeft;
        //if(homeCnt>visitCnt){
...
void calcNextEP(int *arrncEdge, int *arrngEdge,int *arrncEdgeLeft, int *arrngEdgeLeft,int *cpos, int *gpos,int *arrnEdge, int *cvarEdge, int *gvarEdge,int *arrncEdgeMin,int *arrngEdgeMin,int cCnt,int gCnt, int nId,char *home,bool IsStatic){
/*
 ...
 */
    ...
    if(arrnHomeEdgeLeft[rnIdM1]>0){// check whether HomeEdge[R(nId-1)] finish or not
        ...
        calcNextEEL(arrnHomeEdge, arrnHomeEdgeLeft,arrnEdge, varEdge,arrnHomeEdgeMin, rnIdM1, homeCnt, visitCnt,IsStatic);
        ...
        if(arrnHomeEdgeLeft[rnIdP1]>0){// check whether HomeEdge[R(nId+1)] finish or not
            ...
            calcNextEEL(arrnHomeEdge, arrnHomeEdgeLeft,arrnEdge, varEdge,arrnHomeEdgeMin, rnIdP1, homeCnt, visitCnt,IsStatic);
            ...
            if(arrnHomeEdgeLeft[rnId]>0){// check whether Edge[R(nId)] finish or not
                ...
                calcNextEEL(arrnHomeEdge, arrnHomeEdgeLeft,arrnEdge, varEdge,arrnHomeEdgeMin, rnId, homeCnt, visitCnt,IsStatic);
                if(sign == -1){//gpu position
                    homePos[rnId] = homePos[rnId] - arrnHomeEdge[rnId];
                ...
                if(arrnVisitEdgeLeft[rnIdP1]>0){// check whether visit Edge[rnIdP1] finish or not
                    ...
                    if(arrnVisitEdgeLeft[rnIdM1]>0){ // check whether visit Edge[rnIdM1] finish or not, 
                        ...
                        calcNextEEL(arrnHomeEdge, arrnHomeEdgeLeft,arrnEdge, varEdge,arrnHomeEdge, rnIdM1, homeCnt, visitCnt,IsStatic);
                        homePos[rnIdM1] = (sign == 1)? 0 : arrnEdge[rnIdM1] - arrnHomeEdge[rnIdM1] ;
                        arrnVisitEdgeLeft[rnIdM1] = arrnHomeEdgeLeft[rnIdM1]; 
                    }
                }
