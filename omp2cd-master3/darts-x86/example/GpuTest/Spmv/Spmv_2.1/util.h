...
template <typename floatType>
void readMarketMatrixToCSR(char *filename, CSRMM<floatType> *csr );
template <typename floatType>
                ...
                int **rowDelimiters_ptr, int *n, int *size)
{
    ...
    //std::cout<<"nElements is : " << nElements<<std::endl;
...
void readMarketMatrixToCSR(char *filename, CSRMM<floatType> *csr) 
{
...
#ifdef DARTS_DEBUG
    std::cout<<"read matrix to csr begin!"<<std::endl;
#endif
    ...
    // create CSR data structures
    csr->assignNumNonZeroes(nElements);
    csr->assignNumRows(nRows);
    csr->assignNumCols(nRows);//nCols = nRows
...
#ifdef DARTS_DEBUG
    std::cout<<"read matrix finished!"<<std::endl;
#endif
}
...
//   maxi: specifies range of random data
//
// Programmer: Lukasz Wesolowski
...
//    srand48(8675309L);
//
//    // Randomly decide whether entry i,j gets a value, but ensure n data
//    // are assigned
...
//
// Arguments:
//   A: array holding the non-zero data for the matrix
...
//
// Arguments:
//   A: array holding the non-zero data for the matrix in
//      row-major format
...
//
// Purpose: pads a CSR matrix with zeros so that each line of data
...
template<typename T>
double calc_mean(T* data, int start, int end){
    double r =0;
    for(int i=start;i<end;++i){
        r +=data[i];
    }
    return r/double(end-start);
}
//calc standard deviation(sd)
template<typename T>
double calc_sd(T* data, int start, int end){
    double r=0;
    //double u=calc_mean(data,start,end);
    double sum=0;
    double u=0;
    int size = end-start;
    for (int i=start;i<end;++i){
...
//calc coefficient of variation (cv = sd/u)
template<typename T>
double calc_cv(T* data, int start, int end){
    double r=0;
    //double u=calc_mean(data,start,end);
    double sum=0;
    double u=0;
