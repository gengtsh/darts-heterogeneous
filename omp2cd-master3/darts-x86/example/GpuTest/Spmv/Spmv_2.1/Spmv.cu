	     ...
	     const floatType *vec, int dim, floatType *out)
{
    ...
    {
        //if (i == 259752){
        //    printf("cpu: row  %d, total %d cols\n",i,rowDelimiters[i+1]-rowDelimiters[i]);
        //
        // 
        //    floatType t = 0;
        ...
        //        int col = cols[j];
        //        t += val[j] * vec[col];
        //        printf("val[%d] = %f, vec[%ld] = %f, tmp = %f\n",j,val[j],cols[j],vec[col],val[j]*vec[col]);
        //    }
        ...
        int numRows, int numNonZeroes, floatType* refOut, bool padded)
{
      ...
      int *d_cols, *d_rowDelimiters;
        ...
        cout<<"numNonZeroes: "<< numNonZeroes<<", numRows: "<<numRows<<", d_rowDelimiters: "<< (numRows+1)<<endl;
        ...
        int paddedSize)
{
    ...
    std::cout<<"ELL numRows: "<<numRows<<", cmSize = "<<cmSize<<std::endl;
    std::cout<<"ELL h_valcm size = "<<maxrl*cmSize*sizeof(floatType)/(1024*1024)<<std::endl;
    ...
    std::cout<<"gpu allocate finish!"<<std::endl;
    // Transfer data to device
...
void RunTest(ResultDatabase &resultDB, OptionParser &op, int nRows=0)
{
    ...
    //cout << "inFileName is "<< inFileName<<std::endl; 
    ...
    //cout<<"H_vec size: "<<numRows*sizeof(floatType)<<std::endl;
   ...
   //         &h_colsPad, h_rowDelimitersPad, &nItemsPadded);
    cout<<"paddedSize: "<<paddedSize*sizeof(floatType)<<std::endl;
                       ...
                       const int dim, fpType * __restrict__ out)
{
    ...
    //    printf("row: %d, total %d cols \n", myRow, end-start);
    //    for (int j = start; j < end; j++)
    //    {
    //        int col = cols[j];
    //        t += val[j] * vecTexReader(col);
    //        printf("val[%d] = %f, vectexReader(%d) = %f\n",j,val[j],col,vecTexReader(col));
    //    }
    //    printf("out[%d] = %f\n", myRow,t);
    //
    //}
