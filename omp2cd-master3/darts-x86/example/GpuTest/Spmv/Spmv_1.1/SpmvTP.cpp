...
void SpmvGpuCheckCD::fire(void){
    ...
    if(config == "gpu" ){
        ...
        int totalNumRows        =FRAME( totalNumRows     );
        int nRows = (numRowsLeft > 2000)?FRAME(gpuNumRows): numRowsLeft;
        ...
        std::cout<<"totalNumRows: "<<totalNumRows<<std::endl;
        std::cout<<"numRowsLeft: "<<numRowsLeft<<std::endl;
...
#ifdef DARTS_DEBUG
        std::cout<<"device: new start: "<< newStart<<", newNumRows: "<<nRows<<std::endl;
...
//    if((!((config=="cpu")&&(FRAME(IsCpuParallel)==false)))&&(FRAME(IsRefReq == true))){
