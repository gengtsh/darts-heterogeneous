...
DEF_CODELET(SpmvGpuCD,0,SHORTWAIT);
DEF_CODELET(SpmvGpuCheckCD,0,SHORTWAIT);

DEF_CODELET(SpmvGpuStreamCD,0,SHORTWAIT);
DEF_CODELET(SpmvGpuStreamCheckCD,0,SHORTWAIT);
...
DEF_TP(SpmvTP)
{
    ...
    SpmvGpuCD spmvGpu;
    SpmvGpuCheckCD spmvGpuCheck;
    SpmvGpuStreamCD spmvGpuStream;
    SpmvGpuStreamCheckCD spmvGpuStreamCheck;
    SyncCD	sync;
    Codelet *signalUp;
    ...
    SpmvTP(RecordDatabase *recordDB,ResultDatabase *resultDB, OptionParser *op,Codelet *up)
        ...
        ,signalUp(up)
        {
            ...
            if(config != "cpu"){
                if(IsStream == false){
#ifdef DARTS_DEBUG
                    std::cout<<"add spmvGpu!"<<std::endl;
#endif
                    spmvGpu = SpmvGpuCD{dep,1,this,GPUMETA};
                    add(&spmvGpu);
                
#ifdef DARTS_DEBUG
                    std::cout<<"add spmvGpuCheck!"<<std::endl;
#endif
                    spmvGpuCheck = SpmvGpuCheckCD{1,1,this,GPUMETA};
                }else{

#ifdef DARTS_DEBUG
                    std::cout<<"add spmvGpuStream!"<<std::endl;
#endif
                    spmvGpuStream = SpmvGpuStreamCD{dep,1,this,GPUMETA};
                    add(&spmvGpuStream);
                
#ifdef DARTS_DEBUG
                    std::cout<<"add spmvGpuStreamCheck!"<<std::endl;
#endif
                    spmvGpuStreamCheck = SpmvGpuStreamCheckCD{1,1,this,GPUMETA};
