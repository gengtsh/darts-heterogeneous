...
void  CalcMonotonicQRegionForElemsIterCD ::fire(void)
{
	...
	size_t	Id	= getID();
	std::cout<<"CalcMonotonicQRegionForElemsIter["<<Id<<"], in Zone:"<<numZone<<" is running!"<<std::endl; 
...
void CalcMonotonicQRegionForElemsSyncCD::fire(void)
{
	...
	std::cout<<"CalcMonotonicQRegionForElemsSync in Zone:"<<numZone<<" is running!"<<std::endl; 
