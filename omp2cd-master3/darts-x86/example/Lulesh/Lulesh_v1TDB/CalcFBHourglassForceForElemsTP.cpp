...
CalcFBHourglassForceForElemsP1IterCD::fire(void) 
{
	...
	std::cout<<"CalcFBHourglassForceForElemsP1Iter["<<Id<<"] is running!"<<std::endl;
...
{
	std::cout<<"CalcFBHourglassForceForElemsP2CD is running!"<<std::endl;
	LOAD_FRAME(CalcFBHourglassForceForElemsTP);
...
CalcFBHourglassForceForElemsP2IterCD::fire(void) 
{
	...
	std::cout<<"CalcFBHourglassForceForElemsP2Iter["<<Id<<"] is running!"<<std::endl;
	size_t IdC0 = g_treeBarrier*(Id+1);
	size_t IdCE = IdC0+g_treeBarrier;
	size_t tree = MIN(IdCE,N_CORES);
	for (size_t i=IdC0;i<tree;++i){
...
{
	std::cout<<"CalcFBHourglassForceForElemsSyncCD is running!"<<std::endl;
	LOAD_FRAME(CalcFBHourglassForceForElemsTP);
	SIGNAL(signalUp);
	EXIT_TP();
}
