...
//	std::cout <<ss.str();
	...
	uint64_t nChunks=n_rows/g_nSU;

	for(size_t i=0; i<g_nSU; ++i){
		uint64_t pos = nChunks*i*n_cols;
		
        uint64_t  nTpRowsFinal= (i ==(g_nSU-1))?(n_rows-i*nChunks) : (nChunks+2);
        INVOKE(Stencil2DRowDecomposition,initial+pos,nTpRowsFinal,n_cols,ts,i,FRAME(checkTP), &FRAME(sync),FRAME(check[i])); 
