...
{
	std::cout << "USAGE: " << name << "nRows [nCols [nTimeSteps [nReps] ] ]\n";
...
int main(int argc, char *argv[])
{
	...
	if (affin.generateMask()){
		...
		for(size_t i=0; i<nReps;++i){
			...
			rt->run(
                launch<Stencil2DPartition>(InitialMatrix,nRows,nCols,NewMatrix,nTmSteps, &Runtime::finalSignal)
            );
...
//              << outerAvg                << "\n";
	...
	print_results(OriginalMatrix,kk,ttk,jj,ttj,nRows,nCols);
    std::cout<<"Seq Matrix:"<<std::endl;
	print_results(seqOut,kk,ttk,jj,ttj,nRows,nCols);
	std::cout<<"Out Matrix:"<<std::endl;
	print_results(dartsOut,kk,ttk,jj,ttj,nRows,nCols);
#endif
