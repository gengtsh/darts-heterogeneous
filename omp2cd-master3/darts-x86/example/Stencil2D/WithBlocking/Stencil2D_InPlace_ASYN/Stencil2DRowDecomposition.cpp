...
//	fprintf(stderr, "U%ju,ts:%ju\n",Id,ts[Id]);
	...
	const uint64_t rows_ini = n_rows / nRowsCut; // initially, the total number of rows in every nRowsCut
...
//	fprintf(stderr, "D%ju,ts:%ju\n",Id,ts[Id]);
	...
	//uint64_t bn=n_cols - 2;//blockN
	double *share = FRAME(shareRows+Id*2*n_cols);
	const uint64_t rows_ini = n_rows / nRowsCut; 
	uint64_t bm_final = (Id ==(nRowsCut-1))?(n_rows-Id*rows_ini) : (rows_ini+2);
...
//	fprintf(stderr, "C%ju,ts:%ju\n",Id,ts[Id]);
	...
	const uint64_t n_rows = FRAME(nRows); // matrix M row
	const uint64_t n_cols = FRAME(nCols); // Matrix N column
	uint64_t bp= n_cols ;//block position
	//uint64_t bm=n_rows - 2;//blockM
	//uint64_t bn=n_cols - 2;//blockN
	double *share = FRAME(shareRows);
	const uint64_t rows_ini = n_rows / nRowsCut ; // initially, the total number of rows in every nRowsCut
	uint64_t bm_final = (Id ==(nRowsCut-1))?(n_rows-Id*rows_ini) : (rows_ini+2);
	uint64_t dpos = Id*rows_ini * n_cols; 
    uint64_t spos = Id*2*n_cols;	
	computeInner_stencil2dbk(initial+dpos,share+spos,bm_final,n_cols,n_rows,n_cols);
