...
public:
    ...
    /**
         ...
         */
    Codelet(uint32_t dep, uint32_t res, ThreadedProcedure* theTp = NULL,
        uint32_t stat = SHORTWAIT);
    Codelet(uint32_t dep, uint32_t res, ThreadedProcedure* theTp,
    ...
    /**
         ...
         * Implements a delayed Codelet init. We may need them for an array of codelets
				 ...
				 *	stat - Locality parameter (TODO: has to be explicited)
         */
    void initCodelet(uint32_t dep, uint32_t res, ThreadedProcedure* theTp,
        uint32_t stat);
	...
	virtual void decCnt(void); //Geng, just for test 

    inline void incDep(void)
    {
        sync_.incOnlyCounter();
    }
    ...
    /**
         ...
         */
    inline void setStatus(uint32_t stat)
    {
        status_ = stat;
    }
    ...
    inline ThreadedProcedure* getTP(void)
    {
        return myTP_;
    }
    ...
    inline void setLocalID(uint32_t codeletID)
    {
        this->myLocalID = codeletID;
    }

    inline void setGlobalID(uint32_t codeletID)
    {
        this->myGlobalID = codeletID;
    }

    inline void setID(uint32_t codeletID)
    {
        this->myGlobalID = codeletID;
    }

    inline uint32_t omp_get_thread_num()
    {
        return this->myGlobalID;
    }
